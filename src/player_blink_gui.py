...
class PlayerBlinkGUI(tk.Frame):
    ...
    def append_to_clipboard(self, text_to_append):
        ...
        root.clipboard_append(text_to_append)

    def copy_seed_to_clipboard(self, seed_to_copy, text_widget, button_widget):
        seed_data = text_widget.get('1.0', 'end').strip().splitlines()

        if seed_data:
            final_seed = str()

            if len(seed_to_copy) == 1:
                final_seed = seed_data[int(seed_to_copy)]
                self.append_to_clipboard(final_seed)

            else:
                for seed_id in seed_to_copy: # Need to better optimize
                    final_seed += seed_data[int(seed_id)]
            ...
            messagebox.showwarning(title="Attention - Seed Copying Error", 
                             message=rf"Seeds are not available for copying yet."+ "\nPress OK to Continue.")
    ...
    def create_widgets(self):
        ...
        ttk.Label(self,text="S[0-3]:").grid(column=0,row=1)
        ttk.Label(self,text="S[0-1]:").grid(column=0,row=6)
        ...
        self.eye_display = tk.Label(self)
    ...
    def new_eye(self):
        ...
        self.eye_img = Image.open(self.config_json["image"])
        self.eye_resized = self.eye_img.resize((20, 20), Image.Resampling.LANCZOS)
        self.new_eye_img = ImageTk.PhotoImage(self.eye_resized)
        self.eye_display['image'] = self.new_eye_img
